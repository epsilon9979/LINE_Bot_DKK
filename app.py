...
category = ['Disaster','基隆','新北市','臺北市','桃園市','新竹','苗栗縣','臺中市','彰化縣','南投縣','雲林縣','嘉義','臺南市','高雄市','屏東縣','臺東縣','花蓮縣','宜蘭縣','連江縣','金門縣','澎湖縣','國際議題','能源議題','防災資訊']
# Your user ID
...
def handle_message(event):
    ...
    if '\n' in response:
        result = answer(response) # answer(response) = [TextSendMessage(text = f"已超過作答時間"), "empty"] or [flex_message1, "empty"] or [flex_message1, flex_message2]
        if result[1] == "empty":
